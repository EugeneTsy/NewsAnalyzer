...
export class SearchInput extends BaseComponent {
  ...
  validation() {
    ...
    if (_value.length === 0) {
      ...
      INPUT_ERROR.textContent = `Можно максимум 50 символов. Удалите еще ${_value.length -
        50}`;
      INPUT_ERROR.classList.add("searching__error_shown");
      SUBMIT_BTN.classList.add("searching-form__button_moved");
    } else {
  ...
  submit() {
    ...
    if (this._isValid) {
      ...
      cardList
        .renderNews(this.form.elements.searchInput.value, false)
        .finally(() => {
  ...
  _HANDLERS() {
    this.form.elements.searchInput.addEventListener("input", () =>
      this.validation()
    );
